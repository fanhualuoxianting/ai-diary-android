        ...
        item {
            ElevatedCard(Modifier.fillMaxWidth()) {
                Column(
                    ...
                    Text("中文日记草稿", style = MaterialTheme.typography.titleMedium)
                    OutlinedTextField(
    ...
    Card(Modifier.fillMaxWidth()) {
        Column(
            ...
            Row {
                ...
                Text("三餐记录", style = MaterialTheme.typography.titleMedium, fontWeight = FontWeight.SemiBold)
            }
...
private fun HistoryScreen(entries: List<DiaryEntry>, modifier: Modifier = Modifier) {
    LazyColumn(
        ...
        item {
            Text("历史日记", style = MaterialTheme.typography.headlineSmall, fontWeight = FontWeight.Bold)
        }
        if (entries.isEmpty()) {
            item { Text("还没有保存的日记。") }
        } else {
    ...
    Column(
        ...
        Card(Modifier.fillMaxWidth()) {
            Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                Text("本地 Gemma 模型", style = MaterialTheme.typography.titleMedium)
                Text(
                    if (state.modelAvailable) "模型路径已配置。" else "未配置模型时，应用会使用可编辑的本地草稿。",
                    color = MaterialTheme.colorScheme.onSurfaceVariant,
                )
                OutlinedTextField(
                    ...
                    modifier = Modifier.fillMaxWidth(),
                    label = { Text("模型文件路径") },
                    placeholder = { Text("/sdcard/Download/gemma.task") },
                    singleLine = true,
                )
                OutlinedTextField(
